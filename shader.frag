...
void main()
{
    ...
    vec3 half_vector = normalize(light_pos - camera_pos);

    vec3 diffuse_shading = (light_color * clamp(dot(normalize(view_normal), normalize(light_pos - view_pos)), 0, 1) * diffuse_color);
